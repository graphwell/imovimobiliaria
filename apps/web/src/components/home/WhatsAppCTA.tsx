...
const MSG_DEFAULT = 'Olá! Vim pelo IMOV e gostaria de falar com um especialista.'

interface WhatsAppCTAProps {
  numero: string
}

export function WhatsAppCTA({ numero }: WhatsAppCTAProps) {
  const [mensagem, setMensagem] = useState(MSG_DEFAULT)

  useEffect(() => {
    const salva = sessionStorage.getItem('imov_wpp_msg')
    if (salva) setMensagem(salva)
  }, [])
